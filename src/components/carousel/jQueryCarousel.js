...
function createImageListHTML(imageList, targetImage) {
    ...
    for (var i = 0; i < imageList.length; i++) {
        ...
        html.push('' +
            ...
            '   <div class="imageName"> ' +
            '       <div class="ownerUserAvatar">' +
            '           <img  src="' + (ownerUser.avatar || "") + '" alt="">' +
            '       </div>' +
            '       <div class="ownerUserNickname">' + encodeHTML(ownerUser.nickname) + '</div>' +
            '       <div class="imaName">' + encodeHTML(img.name) + '</div>' +
            '   </div>' +
            '   <div class="imageObjWrapper" style="height: ' + imageObjWrapperHeight + 'px">' +
            '       <img class="imageObj" src="' + src + '" osrc="' + img.oSrc + '" alt="' + encodeHTML(img.name) + '" >' +
            '   </div>' +
            '</div>'
        );
    }
